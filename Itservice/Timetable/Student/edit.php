...
if(isset($_REQUEST['update_id']))
{
	try
	{
		...
		$select_stmt = $db->prepare('SELECT * FROM studenttimetable WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update']))
{
	try
	{
		...
		if(!isset($errorMsg))
		{
			$update_stmt=$db->prepare('UPDATE studenttimetable SET class=:name_up, images=:file_up WHERE id=:id'); //sql update query
...
<title>Edit Students Time Table</title>
	...
	<div class="container">
		...
		?>   
			...
			<form method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<div class="col-sm-6">
				<input type="text" name="txt_name" class="form-control" value="<?php echo $class; ?>" required/>
				</div>
				</div>
